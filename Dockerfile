...
RUN useradd --uid 1001 --create-home -ms /bin/bash  -ms /var/ appuser 
...
RUN chown -R appuser:appgroup /usr/local/bin/pip && \
    chmod -R 777 /usr/local/bin/pip
...
RUN pip install  --user torch torchaudio torchvision --index-url https://download.pytorch.org/whl/cpu 
